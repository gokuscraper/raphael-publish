...
import { X, Loader2, CheckCircle2, AlertCircle, ExternalLink, Send, AlertTriangle, LogIn, Globe } from 'lucide-react';
import { COSE_PLATFORMS } from '../lib/platformsCose';
...
export default function CoseSyncDialog({ isOpen, onClose, title, htmlContent, markdownContent }: CoseSyncDialogProps) {
    ...
    const [coseRawAccounts, setCoseRawAccounts] = useState<any[]>([]);
    ...
    useEffect(() => {
        ...
        const cose = (window as any).$cose;
        if (!cose) {
            setExtensionInstalled(false);
            setPlatforms(COSE_PLATFORMS.map((p) => ({ ...p, isAuthenticated: false })));
            setStep('select');
            return;
        }
        setExtensionInstalled(true);

        cose.getAccounts((result: any[]) => {
            const accounts = Array.isArray(result) ? result : [];
            setCoseRawAccounts(accounts);
            const authed = new Set(
                accounts.filter((a: any) => a.loggedIn).map((a: any) => a.type)
            );
            const merged: PlatformItem[] = COSE_PLATFORMS.map((p) => {
                const account = accounts.find((a: any) => a.type === p.type);
                return {
                    ...p,
                    isAuthenticated: authed.has(p.type),
                    username: account?.loggedIn ? (account?.displayName || account?.title) : undefined,
                };
            });
            setPlatforms(merged);

            const volcengineRaw = accounts.find((a: any) => a.type === 'volcengine');
            if (volcengineRaw) {
                volcengineRaw.loggedIn = true;
            }
            const volcenginePlatform = merged.find((p) => p.type === 'volcengine');
            if (volcenginePlatform) {
                volcenginePlatform.isAuthenticated = true;
            }
    ...
    const startSync = useCallback(() => {
        ...
        const targetTypes = new Set(Array.from(selected).filter((t) => {
            const p = platforms.find((x) => x.type === t);
            return p?.isAuthenticated && !p?.disabled;
        }));

        if (targetTypes.size === 0) return;

        const accounts = coseRawAccounts
            .filter((a) => targetTypes.has(a.type))
            .map((a) => ({ ...a, checked: true }));

        if (accounts.length === 0) return;

        setStep('syncing');
        setProgress(accounts.map((a) => ({ type: a.type, title: a.title || a.displayName, status: 'pending' as const })));

        cose.addTask(
            {
                post: { title, content: htmlContent, markdown: markdownContent },
                accounts,
            },
    ...
    }, [platforms, selected, coseRawAccounts, title, htmlContent, markdownContent]);
    ...
    return (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/30 backdrop-blur-sm">
            ...
            >
                <div className="flex items-center justify-between px-5 py-4 border-b border-[#00000010] dark:border-[#ffffff10]">
                    <div className="flex items-center gap-2">
                        ...
                        <h2 className="text-[17px] font-semibold text-[#1d1d1f] dark:text-[#f5f5f7]">COSE 副同步</h2>
                    </div>
                ...
                <div className="px-5 py-4 max-h-[60vh] overflow-y-auto">
                    {extensionInstalled === false && (
                        <div className="flex flex-col items-center gap-3 py-8 text-center">
                            ...
                            <p className="text-[15px] text-[#1d1d1f] dark:text-[#f5f5f7] font-medium">未检测到 COSE 扩展</p>
                            <p className="text-[13px] text-[#86868b]">请先安装 COSE（多平台文章同步）Chrome 扩展</p>
                        </div>
                    )}
            ...
            <div className="text-[12px] text-[#86868b] truncate">
                                                {isDisabled ? '暂不支持' : isAuthed ? (p.type === 'volcengine' ? '强制已登录（检测有bug）' : p.username ? `@${p.username}` : '已登录') : '未登录，点击去登录'}
                                            </div>
                                            </div>
                                            {!isAuthed && !isDisabled && (
                            ...
                            <div className="mt-3 flex items-center gap-1.5 text-[11px] text-[#86868b] justify-center">
                                <Globe size={12} />
                                <span>由 COSE 扩展驱动</span>
                            </div>
                        </>
                    )}
